...
# catcon function ---------------------------------------------------------
catcont = function(list_of_variables=c(),data=c(),group=c(),which.group=c(),
    formal.test=c(),digits=2,caption='Summary Table'){
    
    # keep an untouched part of the data to re-use in the NA subsetting
    full_data = data 
  ...
  seperate_info = vector('list',length(list_of_variables))
    ...
    if (group==FALSE){
        ...
        # only work with non-missing data on the variable of interest
        data = full_data[!is.na(full_data[,list_of_variables[i]]),] 
        ...
        if(is.numeric(data[,list_of_variables[i]]) | is.integer(data[,list_of_variables[i]])){
            ...
            TemplateMatrix_OneGroup_Continuous[,2] = c('','N','mean (std)',
                'median (IQR)','range')
            ...
            seperate_info[[i]][1,1] = paste0('**',ifelse(label(data[,list_of_variables[i]])=='',
                list_of_variables[i],label(data[,list_of_variables[i]])),'**')
            
            seperate_info[[i]][2,3] = nrow(data)
            
            seperate_info[[i]][3,3] = paste0(round(mean(data[,
                list_of_variables[i]],na.rm=TRUE),digits),'(',round(sd(data[,
                    list_of_variables[i]],na.rm=TRUE),digits),')') 
            
            seperate_info[[i]][4,3] = paste0(round(median(data[,list_of_variables[i]],
                na.rm = TRUE),digits),'(',paste0(round(quantile(data[,list_of_variables[i]],c(.25),
                    na.rm=TRUE),digits),';',round(quantile(data[,list_of_variables[i]],
                        c(.75),na.rm=TRUE),digits)),')')
            
            
            seperate_info[[i]][5,3] = paste0(round(range(data[,
                list_of_variables[i]],na.rm=TRUE)[1],digits),',',
                round(range(data[,list_of_variables[i]],na.rm=TRUE)[2],digits)) 
        }    
      
        if(is.factor(data[,list_of_variables[i]]) | is.character(data[,list_of_variables[i]]) ){
            ## Discrete --------------------------------------------------------------
            TemplateMatrix_OneGroup_Discrete = matrix(c(''),
                nlevels(data[,list_of_variables[i]])+1,3)
            
            TemplateMatrix_OneGroup_Discrete[1,1] = paste0('**',
                ifelse(label(data[,list_of_variables[i]])=='',list_of_variables[i],
                    label(data[,list_of_variables[i]])),'**')
            
            TemplateMatrix_OneGroup_Discrete[2:(nlevels(data[,
                list_of_variables[i]])+1),2] = 'n/N (%)'
        ...
        for(level in 1:nlevels(data[,list_of_variables[i]])){
          ...
          data[,'count_dummy'] = ifelse(data[,list_of_variables[i]]==levels(data[,
              list_of_variables[i]])[level],1,0)
          
          seperate_info[[i]][level+1,1] = levels(data[,list_of_variables[i]])[level]
          
          seperate_info[[i]][level+1,3] = paste0(sum(data[,'count_dummy']),'/',
              nrow(data),'(',round((sum(data[,'count_dummy'])/nrow(data))*100,2),'%',')')
                }
            }
        }
    ...
    if (group==TRUE){
        ...
        ## Continuous --------------------------------------------------------------
            ...
            TemplateMatrix_MultipleGroups_Continuous = matrix(c(''),5,
                nlevels(data[,which.group])+3)
            
            colnames(TemplateMatrix_MultipleGroups_Continuous) = 
                c('variable','statistic','total',levels(data[,which.group]))
            
            TemplateMatrix_MultipleGroups_Continuous[1,1] = paste0('**',
                ifelse(label(data[,list_of_variables[i]])=='',list_of_variables[i],
                    label(data[,list_of_variables[i]])),'**')
            
            TemplateMatrix_MultipleGroups_Continuous[2:5,2] = 
                c('N','mean (std)','median (IQR)','range')
            ...
            seperate_info[[i]][3,3] = paste0(round(mean(data[,list_of_variables[i]],
                na.rm=TRUE),2),'(',round(sd(data[,list_of_variables[i]],na.rm=TRUE),digits),')') 
            
            seperate_info[[i]][4,3] = paste0(round(median(data[,list_of_variables[i]]),
                digits),'(',paste0(round(quantile(data[,list_of_variables[i]],c(.25),
                    na.rm=TRUE),digits),';',round(quantile(data[,list_of_variables[i]],
                        c(.75),na.rm=TRUE),digits)),')')
        
            seperate_info[[i]][5,3] = paste0(round(range(data[,list_of_variables[i]],
                na.rm=TRUE)[1],digits),',',round(range(data[,list_of_variables[i]],
                    na.rm=TRUE)[2],digits)) 
